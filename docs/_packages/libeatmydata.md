...
5 / 47 files match
 ...
 - [m4/pandora_have_libdl.m4](#m4pandora_have_libdlm4)
...
15 |   AC_CHECK_FUNCS(dlopen)
20 |   AM_CONDITIONAL(HAVE_LIBDL, [test "x${ac_cv_func_dlopen}" = "xyes"])
43 |   AS_IF([test "$ac_cv_func_dlopen" != "yes"],[
44 |     PANDORA_MSG_ERROR([libdl/dlopen() is required for ${PACKAGE}. On Debian this can be found in libc6-dev. On RedHat this can be found in glibc-devel.])
{% endraw %}
