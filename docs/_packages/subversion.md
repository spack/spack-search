...
name: "subversion"
layout: package
next_package: libevpath
previous_package: vtk-h
languages: ['bash']
---
## 1.9.6
6 / 1931 files match
 ...
 - [build/ltoptions.m4](#buildltoptionsm4)
 - [build/ltmain.sh](#buildltmainsh)
 - [build/ac-macros/sqlite.m4](#buildac-macrossqlitem4)
 - [tools/dev/unix-build/Makefile.svn](#toolsdevunix-buildmakefilesvn)
...
{% raw %}
198 | LT_EXECUTE = $(LIBTOOL) $(LTFLAGS) --mode=execute `for f in $(abs_builddir)/subversion/*/*.la; do echo -dlopen $$f; done`
{% endraw %}
...
273 |                 [lt_cv_dlopen="dlopen" svn_sqlite_dso_ldflags="-ldl"],[
283 |         [AC_CHECK_FUNC([dlopen],
285 |           [AC_CHECK_LIB([dl], [dlopen],
287 |             [AC_CHECK_LIB([svld], [dlopen],
{% endraw %}
...
{% raw %}
948 | 		< $(CYRUS_SASL_SRCDIR)/lib/dlopen.c \
949 | 		> $(CYRUS_SASL_SRCDIR)/lib/dlopen.c.patched
950 | 	mv $(CYRUS_SASL_SRCDIR)/lib/dlopen.c.patched \
951 | 		$(CYRUS_SASL_SRCDIR)/lib/dlopen.c
1566 | # OpenBSD requires an LD_PRELOAD hack to dlopen() libraries linked to
{% endraw %}
