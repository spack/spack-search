...
7 / 157 files match
 ...
 - [ltmain.sh](#ltmainsh)
 - [src/lib/modules.c](#srclibmodulesc)
...
{% raw %}
129 | AC_CHECK_LIB(dl, dlopen, DLOPEN_LIBS=-ldl)
130 | AC_SUBST(DLOPEN_LIBS)
{% endraw %}
...
{% raw %}
82 |     * dlopen can take an extension-less name and do the Right Thing
{% endraw %}
...
{% raw %}
562 | /* try dlopen()ing the file if we succeed finish filling out the malloced */
573 |    l->handle = dlopen(file, RTLD_NOW | RTLD_LOCAL);
596 | /* fre the struct for a loader and close its dlopen'd handle */
{% endraw %}
