...
name: "npth"
layout: package
next_package: xcb-util-renderutil
previous_package: heppdt
languages: ['bash']
---
## 1.6
3 / 41 files match
...
6823 | 		    # if the lib is a (non-dlopened) module then we can not
6827 | 		      if test "X$dlopenmodule" != "X$lib"; then
6979 | 	      echo "*** a static module, that should work as long as the dlopening application"
6980 | 	      echo "*** is linked with the -dlopen flag to resolve symbols at runtime."
7124 |       if test "$pass" != dlopen; then
7223 | 	func_warning "\`-dlopen' is ignored for archives"
7290 | 	func_warning "\`-dlopen self' is ignored for libtool libraries"
7977 | 	    echo "*** a static module, that should work as long as the dlopening"
7978 | 	    echo "*** application is linked with the -dlopen flag."
7996 | 	    echo "*** or is declared to -dlopen it."
8607 | 	func_warning "\`-dlopen' is ignored for objects"
8725 |         && test "$dlopen_support" = unknown \
8726 | 	&& test "$dlopen_self" = unknown \
8727 | 	&& test "$dlopen_self_static" = unknown && \
8728 | 	  func_warning "\`LT_INIT([dlopen])' not used. Assuming no dlopen support."
9409 | # The name that we can dlopen(3).
9438 | # Files to dlopen/dlpreopen
9439 | dlopen='$dlfiles'
{% endraw %}
