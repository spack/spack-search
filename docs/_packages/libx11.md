...
{% raw %}
89 | # Check for dlopen
91 | AC_SEARCH_LIBS(dlopen,[dl svld])
92 | if test "x$ac_cv_search_dlopen" = xno; then
100 | 	AC_DEFINE(HAVE_DLOPEN,1,[Use dlopen to load shared libraries])
{% endraw %}
...
{% raw %}
20 | /* Use dlopen to load shared libraries */
...
{% raw %}
285 | /* We reference count dlopen() and dlclose() of modules; unfortunately,
