...
14 / 2186 files match
 ...
 - [libglusterfs/src/glusterfs/xlator.h](#libglusterfssrcglusterfsxlatorh)
 - [glusterfsd/src/glusterfsd.c](#glusterfsdsrcglusterfsdc)
 - [tests/features/ipc.t](#testsfeaturesipct)
 - [rpc/rpc-lib/src/rpc-transport.h](#rpcrpc-libsrcrpc-transporth)
 - [rpc/rpc-lib/src/rpc-transport.c](#rpcrpc-libsrcrpc-transportc)
 - [xlators/mgmt/glusterd/src/glusterd-utils.c](#xlatorsmgmtglusterdsrcglusterd-utilsc)
...
{% raw %}
341 | AC_CHECK_LIB([dl], [dlopen], [LIB_DL=-ldl])
475 | AC_CHECK_FUNC([dlopen], [has_dlopen=yes], AC_CHECK_LIB([dl], [dlopen], , AC_MSG_ERROR([Dynamic linking library required to build glusterfs])))
{% endraw %}
...
{% raw %}
1966 |  *                   -1 when other critical errors (dlopen () etc)
1993 |     libhandle = dlopen(libpathfull, RTLD_NOW);
2059 |  *                   -1 when other critical errors (dlopen () etc)
...
{% raw %}
25 | # use libglusterfs's version, and dlopen() does not make any guarantee
{% endraw %}
...
{% raw %}
2263 |      * there's no dlopen interface to get at it, so we assume a mismatch.
{% endraw %}
