...
{% raw %}
46 | LT_INIT([disable-static dlopen pic-only])
70 | THEPEG_CHECK_DLOPEN
{% endraw %}
...
{% raw %}
221 | AC_DEFUN([THEPEG_CHECK_DLOPEN],
222 | [echo $ECHO_N "checking for dlopen... $ECHO_C" 1>&6
225 | ]], [[dlopen("", 1);
226 | ]])],[AC_DEFINE(ThePEG_HAS_DLOPEN,1,define if dlopen is available)
{% endraw %}
...
{% raw %}
147 | /* define if dlopen is available */
148 | #undef ThePEG_HAS_DLOPEN
{% endraw %}
...
{% raw %}
23 | #ifdef ThePEG_HAS_DLOPEN
71 | #ifdef ThePEG_HAS_DLOPEN
