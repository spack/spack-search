...
{% raw %}
142 |     # On platforms with dlopen, we load everything dynamically and
...
{% raw %}
6 | It hides the complexity of `dlopen()`, `dlsym()`, `glXGetProcAddress()`,
{% endraw %}
...
{% raw %}
31 |  * It hides the complexity of `dlopen()`, `dlsym()`, `glXGetProcAddress()`,
220 |      * Locking for making sure we don't double-dlopen().
225 |     /* dlopen() return value for libGL.so.1. */
229 |      * dlopen() return value for OS X's GL library.
235 |     /* dlopen() return value for libEGL.so.1 */
238 |     /* dlopen() return value for libGLESv1_CM.so.1 */
241 |     /* dlopen() return value for libGLESv2.so.2 */
...
301 |         *handle = dlopen(lib_name, RTLD_LAZY | RTLD_LOCAL);
...
{% raw %}
27 | /* Call the *real* dlopen. We have our own wrapper for dlopen that, of
28 |  * necessity must use claim the symbol 'dlopen'. So whenever anything
29 |  * internal needs to call the real, underlying dlopen function, the
...
{% raw %}
46 |  * Wraps the system dlopen(), which libepoxy will end up calling when
47 |  * it tries to dlopen() the API libraries, and errors out the
...
{% raw %}
23 |  * Implements a wrapper for dlopen() and dlsym() so that epoxy will
26 |  * "override_GL_glWhatever()" when it tries to dlopen() and dlsym()
...
170 | /* Since we redirect dlopens of libGL.so and libEGL.so to libfips we
{% endraw %}
