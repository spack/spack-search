...
10 / 650 files match
 ...
 - [extlib/otf/config/ltmain.sh](#extlibotfconfigltmainsh)
 - [extlib/otf/config/m4/libtool.m4](#extlibotfconfigm4libtoolm4)
 - [extlib/otf/config/m4/ltoptions.m4](#extlibotfconfigm4ltoptionsm4)
...
2352 | This mode sets the library path environment variable according to \`-dlopen'
2407 |   -dlopen FILE      \`-dlpreopen' FILE if it cannot be dlopened at runtime
2416 |   -module           build a library that can dlopened
2522 |     # Handle -dlopen flags immediately.
2523 |     for file in $opt_dlopen; do
2542 | 	# Skip this library if it cannot be dlopened.
2569 | 	func_warning "\`-dlopen' is ignored for non-libtool libraries and objects"
5170 | 	    dlopen_self=$dlopen_self_static
5176 | 	    dlopen_self=$dlopen_self_static
5182 | 	    dlopen_self=$dlopen_self_static
5240 | 	    elif test "$prev" = dlfiles && test "$dlopen_self" != yes; then
5324 | 		    if test "$build_libtool_libs" = yes && test "$dlopen_support" = yes; then
5486 |       -dlopen)
5888 | 	      if test "$build_libtool_libs" = yes && test "$dlopen_support" = yes; then
5956 | 	  # This library was specified with -dlopen.
6073 | 	    func_fatal_help "libraries can \`-dlopen' only libtool libraries: $file"
6084 | 	passes="conv scan dlopen dlpreopen link"
6110 | 	dlopen) libs="$dlfiles" ;;
6138 |       if test "$pass" = dlopen; then
6356 | 	    if test "$pass" = dlpreopen || test "$dlopen_support" != yes || test "$build_libtool_libs" = no; then
6357 | 	      # If there is no dlopen support or we're linking statically,
6387 | 	dlopen=
6417 | 	  test -n "$dlopen" && func_append dlfiles " $dlopen"
6463 | 	# This library was specified with -dlopen.
6464 | 	if test "$pass" = dlopen; then
6466 | 	    func_fatal_error "cannot -dlopen a convenience library: \`$lib'"
6469 | 	     test "$dlopen_support" != yes ||
6471 | 	    # If there is no dlname, no dlopen support or we're linking
6480 | 	fi # $pass = dlopen
6536 | 	      # (otherwise, the dlopen module name will be incorrect).  We do
6538 | 	      # We recover the dlopen module name by 'saving' the la file
6562 | 	      # Otherwise, use the dlname, so that lt_dlopen finds it.
6691 | 	  # systems (darwin).  Don't bleat about dlopened modules though!
6692 | 	  dlopenmodule=""
6695 | 	      dlopenmodule="$dlpremoduletest"
6699 | 	  if test -z "$dlopenmodule" && test "$shouldnotlink" = yes && test "$pass" = link; then
6796 | 		    # if the lib is a (non-dlopened) module then we can not
6800 | 		      if test "X$dlopenmodule" != "X$lib"; then
6952 | 	      echo "*** a static module, that should work as long as the dlopening application"
6953 | 	      echo "*** is linked with the -dlopen flag to resolve symbols at runtime."
7097 |       if test "$pass" != dlopen; then
7196 | 	func_warning "\`-dlopen' is ignored for archives"
7263 | 	func_warning "\`-dlopen self' is ignored for libtool libraries"
7938 | 	    echo "*** a static module, that should work as long as the dlopening"
7939 | 	    echo "*** application is linked with the -dlopen flag."
7957 | 	    echo "*** or is declared to -dlopen it."
8567 | 	func_warning "\`-dlopen' is ignored for objects"
8685 |         && test "$dlopen_support" = unknown \
8686 | 	&& test "$dlopen_self" = unknown \
8687 | 	&& test "$dlopen_self_static" = unknown && \
8688 | 	  func_warning "\`LT_INIT([dlopen])' not used. Assuming no dlopen support."
9366 | # The name that we can dlopen(3).
9395 | # Files to dlopen/dlpreopen
9396 | dlopen='$dlfiles'
{% endraw %}
...
```
### extlib/otf/config/m4/libtool.m4
...
```
### extlib/otf/config/m4/ltoptions.m4
