...
{% raw %}
2388 |   void *(*xDlOpen)(sqlite3_vfs*, const char *zFilename);
15958 | SQLITE_PRIVATE void *sqlite3OsDlOpen(sqlite3_vfs *, const char *);
22364 | SQLITE_PRIVATE void *sqlite3OsDlOpen(sqlite3_vfs *pVfs, const char *zPath){
22365 |   return pVfs->xDlOpen(pVfs, zPath);
38762 | static void *unixDlOpen(sqlite3_vfs *NotUsed, const char *zFilename){
38764 |   return dlopen(zFilename, RTLD_NOW | RTLD_GLOBAL);
38769 | ** unixDlOpen() fails (returns a null pointer). If a more detailed error
38812 |   #define unixDlOpen  0
40195 |     unixDlOpen,           /* xDlOpen */                     \
46187 | static void *winDlOpen(sqlite3_vfs *pVfs, const char *zFilename){
46194 |     OSTRACE(("DLOPEN name=%s, handle=%p\n", zFilename, (void*)0));
46199 |     OSTRACE(("DLOPEN name=%s, handle=%p\n", zFilename, (void*)0));
46209 |     OSTRACE(("DLOPEN name=%s, handle=%p\n", zFilename, (void*)0));
46224 |   OSTRACE(("DLOPEN name=%s, handle=%p\n", zFilename, (void*)h));
46246 |   #define winDlOpen  0
46464 |     winDlOpen,             /* xDlOpen */
46489 |     winDlOpen,             /* xDlOpen */
46514 |     winDlOpen,             /* xDlOpen */
46539 |     winDlOpen,             /* xDlOpen */
46674 | static void *memdbDlOpen(sqlite3_vfs*, const char *zFilename);
46695 |   memdbDlOpen,                 /* xDlOpen */
46998 | static void *memdbDlOpen(sqlite3_vfs *pVfs, const char *zPath){
46999 |   return ORIGVFS(pVfs)->xDlOpen(ORIGVFS(pVfs), zPath);
119925 |   handle = sqlite3OsDlOpen(pVfs, zFile);
119930 |     handle = sqlite3OsDlOpen(pVfs, zAltFile);
192705 | static void *rbuVfsDlOpen(sqlite3_vfs *pVfs, const char *zPath){
192707 |   return pRealVfs->xDlOpen(pRealVfs, zPath);
192808 |     rbuVfsDlOpen,                 /* xDlOpen */
{% endraw %}
