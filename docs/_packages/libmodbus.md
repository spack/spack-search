...
6117 |     [Compiler flag to allow reflexive dlopens])
...
6575 | 	    dlopen_self=$dlopen_self_static
6581 | 	    dlopen_self=$dlopen_self_static
6587 | 	    dlopen_self=$dlopen_self_static
6645 | 	    elif test dlfiles = "$prev" && test yes != "$dlopen_self"; then
6735 | 		    if test yes = "$build_libtool_libs" && test yes = "$dlopen_support"; then
6902 |       -dlopen)
7336 | 	      if test yes = "$build_libtool_libs" && test yes = "$dlopen_support"; then
7404 | 	  # This library was specified with -dlopen.
7524 | 	    func_fatal_help "libraries can '-dlopen' only libtool libraries: $file"
7535 | 	passes="conv scan dlopen dlpreopen link"
7561 | 	dlopen) libs=$dlfiles ;;
7589 |       if test dlopen = "$pass"; then
7809 | 	    if test dlpreopen = "$pass" || test yes != "$dlopen_support" || test no = "$build_libtool_libs"; then
7810 | 	      # If there is no dlopen support or we're linking statically,
7838 | 	dlopen=
7868 | 	  test -n "$dlopen" && func_append dlfiles " $dlopen"
7914 | 	# This library was specified with -dlopen.
7915 | 	if test dlopen = "$pass"; then
7917 | 	    && func_fatal_error "cannot -dlopen a convenience library: '$lib'"
7919 | 	     test yes != "$dlopen_support" ||
7922 | 	    # If there is no dlname, no dlopen support or we're linking
7931 | 	fi # $pass = dlopen
7987 | 	      # (otherwise, the dlopen module name will be incorrect).  We do
7989 | 	      # We recover the dlopen module name by 'saving' the la file
8013 | 	      # Otherwise, use the dlname, so that lt_dlopen finds it.
8142 | 	  # systems (darwin).  Don't bleat about dlopened modules though!
8143 | 	  dlopenmodule=
8146 | 	      dlopenmodule=$dlpremoduletest
8150 | 	  if test -z "$dlopenmodule" && test yes = "$shouldnotlink" && test link = "$pass"; then
8247 | 		    # if the lib is a (non-dlopened) module then we cannot
8251 | 		      if test "X$dlopenmodule" != "X$lib"; then
8403 | 	      echo "*** a static module, that should work as long as the dlopening application"
8404 | 	      echo "*** is linked with the -dlopen flag to resolve symbols at runtime."
8548 |       if test dlopen != "$pass"; then
8678 | 	func_warning "'-dlopen' is ignored for archives"
8745 | 	|| func_warning "'-dlopen self' is ignored for libtool libraries"
9439 | 	    echo "*** a static module, that should work as long as the dlopening"
9440 | 	    echo "*** application is linked with the -dlopen flag."
9458 | 	    echo "*** or is declared to -dlopen it."
10070 | 	func_warning "'-dlopen' is ignored for objects"
10190 | 	&& test unknown,unknown,unknown = "$dlopen_support,$dlopen_self,$dlopen_self_static" \
10191 | 	&& func_warning "'LT_INIT([dlopen])' not used. Assuming no dlopen support."
10872 | # The name that we can dlopen(3).
10901 | # Files to dlopen/dlpreopen
10902 | dlopen='$dlfiles'
{% endraw %}
