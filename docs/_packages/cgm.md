...
name: "cgm"
layout: package
next_package: libxprintapputil
previous_package: lksctp-tools
languages: ['bash']
---
## 16.0
5 / 845 files match
 ...
 - [src/util/CubitDynamicLoader.cpp](#srcutilcubitdynamicloadercpp)
...
{% raw %}
45 | LT_INIT([dlopen])
282 |   AC_CHECK_LIB(dl, dlopen, LIBS="$LIBS -ldl")
{% endraw %}
...
```
### src/util/CubitDynamicLoader.cpp

```

{% raw %}
205 |     return dlopen(lib, RTLD_LAZY);
213 |       LibraryHandle handle = dlopen(path.c_str(), RTLD_LAZY);
220 |   return dlopen(lib, RTLD_LAZY );
{% endraw %}
