...
8 / 37314 files match
 ...
 - [adm/cmake/TKernel/CMakeLists.txt](#admcmaketkernelcmakeliststxt)
 - [test/TDataXtd_test/CMakeLists.txt](#testtdataxtd_testcmakeliststxt)
 - [test/gtest-1.7.0/m4/libtool.m4](#testgtest-170m4libtoolm4)
 - [test/gtest-1.7.0/m4/ltoptions.m4](#testgtest-170m4ltoptionsm4)
 - [test/gtest-1.7.0/build-aux/ltmain.sh](#testgtest-170build-auxltmainsh)
 - [test/OCAFExport_test/CMakeLists.txt](#testocafexport_testcmakeliststxt)
...
91 | // DlOpen:   The dlopen function provides an interface to the dynamic 
94 | // The dlopen function attempts to load filename, in the address space 
104 | // symbol binding during the dlopen call.  
105 | // The dlopen function returns a handle that is used by dlsym or 
108 | // If a NULL filename is specified, dlopen returns a handle for the main      
112 | Standard_Boolean  OSD_SharedLibrary::DlOpen(const OSD_LoadMode aMode ) {
114 |   myHandle = dlopen (myName,RTLD_LAZY);
117 |   myHandle = dlopen (myName,RTLD_NOW);
159 | // occurred from a call to dlopen, dlclose or dlsym.
241 | Standard_Boolean OSD_SharedLibrary :: DlOpen ( const OSD_LoadMode /*Mode*/ ) {
258 | }  // end OSD_SharedLibrary :: DlOpen
{% endraw %}
...
{% raw %}
139 |   myGlLibHandle = dlopen ("/System/Library/Frameworks/OpenGL.framework/Versions/Current/OpenGL", RTLD_LAZY);
{% endraw %}
...
{% raw %}
59 | # Modules are dlopen'd by OSD_SharedLibrary::DlOpen.  ELF libraries may
{% endraw %}
...
{% raw %}
1 | 	# This test will dlopen FWOSPlugin, we link against it to ensure that
{% endraw %}

```
### test/gtest-1.7.0/m4/libtool.m4
...
```
### test/gtest-1.7.0/m4/ltoptions.m4
...
```
### test/gtest-1.7.0/build-aux/ltmain.sh
...
{% raw %}
1 | 	# This test will dlopen FWOSPlugin and TKBin, we link against them to ensure that
{% endraw %}
