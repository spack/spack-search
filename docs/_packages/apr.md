...
{% raw %}
2114 |           # use a platform implementation and shouldn't test the dlopen/dlfcn
2117 |           # detection of dlopen/dlsym, do not move them up.
2123 |       AC_CHECK_FUNC(dlopen, [dsotype=dlfcn])
2126 |       AC_CHECK_LIB(dl, dlopen, [dsotype=dlfcn; APR_ADDTO(LIBS,-ldl)])
2129 |         # ReliantUnix has dlopen() in libc but dlsym() in libdl :(
2134 |               echo "Weird: dlopen() was found but dlsym() was not found!"])])
{% endraw %}
...
{% raw %}
75 |  * Mode flags for the dlopen routine.
130 |     void *os_handle = dlopen((char *)path, RTLD_NOW | RTLD_GLOBAL);
180 |  * We simulate dlopen() et al. through a call to load. Because AIX has
203 |  * The void * handle returned from dlopen is actually a ModulePtr.
