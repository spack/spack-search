...
{% raw %}
2274 |   void *(*xDlOpen)(sqlite3_vfs*, const char *zFilename);
14750 | SQLITE_PRIVATE void *sqlite3OsDlOpen(sqlite3_vfs *, const char *);
20851 | SQLITE_PRIVATE void *sqlite3OsDlOpen(sqlite3_vfs *pVfs, const char *zPath){
20852 |   return pVfs->xDlOpen(pVfs, zPath);
36533 | static void *unixDlOpen(sqlite3_vfs *NotUsed, const char *zFilename){
36535 |   return dlopen(zFilename, RTLD_NOW | RTLD_GLOBAL);
36540 | ** unixDlOpen() fails (returns a null pointer). If a more detailed error
36583 |   #define unixDlOpen  0
37966 |     unixDlOpen,           /* xDlOpen */                     \
43830 | static void *winDlOpen(sqlite3_vfs *pVfs, const char *zFilename){
43837 |     OSTRACE(("DLOPEN name=%s, handle=%p\n", zFilename, (void*)0));
43842 |     OSTRACE(("DLOPEN name=%s, handle=%p\n", zFilename, (void*)0));
43852 |     OSTRACE(("DLOPEN name=%s, handle=%p\n", zFilename, (void*)0));
43867 |   OSTRACE(("DLOPEN name=%s, handle=%p\n", zFilename, (void*)h));
43889 |   #define winDlOpen  0
44110 |     winDlOpen,             /* xDlOpen */
44135 |     winDlOpen,             /* xDlOpen */
44160 |     winDlOpen,             /* xDlOpen */
44185 |     winDlOpen,             /* xDlOpen */
113082 |   handle = sqlite3OsDlOpen(pVfs, zFile);
113087 |     handle = sqlite3OsDlOpen(pVfs, zAltFile);
175072 | static void *rbuVfsDlOpen(sqlite3_vfs *pVfs, const char *zPath){
